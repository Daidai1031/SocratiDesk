...
# ═══════════════════════════════════════════
# Local TTS Engine for deterministic playback
# ═══════════════════════════════════════════

class LocalTTS:
    """Local text-to-speech engine for deterministic Stage 1 playback.
    Uses pyttsx3 (offline) with espeak fallback.
    Generates PCM audio and feeds it to the SpeakerPlayer."""
    ...
    def _try_init(self):
        """Try to initialize pyttsx3. Falls back to espeak subprocess."""
        try:
            import pyttsx3
            ...
            for v in voices:
                if 'female' in v.name.lower() or 'en' in v.id.lower():
            ...
            # Check if espeak is available as fallback
            try:
                import subprocess
                ...
                if result.returncode == 0:
                    ...
                    print("[TTS] espeak not available either")
            except Exception:
                print("[TTS] No TTS engine available — will use silent mode")

    def speak_to_file(self, text: str, output_path: str) -> bool:
        """Generate speech audio to a WAV file. Returns True on success."""
        with self._lock:
    ...
    def speak_blocking(self, text: str) -> bool:
        """Speak text directly through the default audio output. Returns True on success."""
        with self._lock:
            if not self._available:
                return False
            try:
                ...
                print(f"[TTS] speak_blocking error: {e}")
                return False
...
class AssistantState:
    def __init__(self):
        ...
        self.tts_speaking = False  # True while local TTS is playing
...
# ═══════════════════════════════════════════
# Local TTS playback handler
# ═══════════════════════════════════════════

async def _play_tts_and_continue(ws, text: str):
    """Play deterministic text via local TTS, then auto-start next recording.
    Pauses mic during playback to prevent echo pickup.
    Runs in a background thread to avoid blocking the event loop."""
    state.tts_speaking = True
    print(f"\n  [TTS] Speaking: {text[:60]}...")

    # PAUSE mic during TTS to prevent echo/feedback
    if _mic_ref is not None:
        _mic_ref.stop()
        print(f"  [TTS] Mic paused during playback")

    # Run TTS in a thread to avoid blocking
    loop = asyncio.get_event_loop()
    success = await loop.run_in_executor(None, local_tts.speak_blocking, text)

    if not success:
        print(f"  [TTS] Playback failed — printing text instead")
        print(f"\n  +--------------------------------------------------+")
    ...
    if _mic_ref is not None:
        ...
        _mic_ref.start()
        print(f"  [TTS] Mic resumed")

    # Longer pause — student needs time to find and read the page
    await asyncio.sleep(2.0)

    # Auto-start next recording turn (student reads their book, then speaks)
    if state.phase == PHASE_TEXTBOOK and _ws_ref is not None:
        print(f"  [AUTO-LISTEN] TTS done, starting next turn for stage={state.stage}")
...
async def _wait_then_listen(ws):
    ...
    # If local TTS is speaking, wait for it to finish
    if state.tts_speaking:
        waited = 0
        while state.tts_speaking and waited < 30.0:
            await asyncio.sleep(0.1)
            waited += 0.1
        await asyncio.sleep(1.0)
        return  # _play_tts_and_continue handles auto-listen
...
async def _do_start_recording(ws):
    ...
    # Note: Textbook stage 1 is now handled deterministically on the server.
    # When begin_turn returns for stage 1, the server has already sent tts_speak
    # and turn_complete. The receiver handles tts_speak → local TTS → auto-listen.
...
async def receiver(ws, speaker, vosk):
    while state.running:
        ...
        # ═══════════════════════════════════════════════
        # NEW: Handle deterministic TTS from server
        # This is used for textbook Stage 1 — no Gemini involved
        # ═══════════════════════════════════════════════
        elif t == "tts_speak":
            text = data.get("text", "")
            if text:
                print(f"\n  [TTS-STAGE1] Deterministic playback (no Gemini)")
                tft.show_status("Open your book!", f"Stage 1/3")
                # Launch TTS playback in background — it will auto-listen when done
                asyncio.ensure_future(_play_tts_and_continue(ws, text))
            ...
            # But NOT if tts_speak is handling playback (it does its own auto-listen)
            if state.tts_speaking:
                print(f"  [AUTO-LISTEN] Skipping — TTS is handling playback")
            else:
                AUTO_LISTEN_PHASES = [
