"""
SocratiDesk Live Server — v5.0
Deterministic Stage 1: No Gemini generation for textbook page direction.
...
import sys
print("=" * 60)
print("  SocratiDesk Server v5.1  —  Deterministic Stage1 + Improved RAG")
print("=" * 60, flush=True)
sys.stdout.flush()
# ═══════════════════════════════════════════════════════════
...
# ═══════════════════════════════════
# Textbook RAG — Two-level retrieval
# ═══════════════════════════════════
...
class TextbookStore:
    ...
    def add_book(self, book_id, name, pages):
        ...
        for p in pages:
            ...
            # Store full lowered text for substring/phrase matching
            page_index.append({
                "page": p["page"],
                "preview": first_line,
                "text_lower": text_lower,
                "keywords": set(re.findall(r"\w{3,}", text_lower)),  # min 3 chars to avoid noise
        ...
        print(f"[RAG] Indexed '{name}': {len(pages)} pages, {len(chunks)} chunks")
        return len(chunks)

    @staticmethod
    def _extract_bigrams(text_lower: str) -> set:
        """Extract adjacent word pairs for phrase matching."""
    ...
    def search_pages(self, query, top_k=3):
        """Stage 1 retrieval: returns page numbers + preview text.
        Uses multi-signal scoring:
          - Exact phrase match in page text (highest weight)
          - Bigram matches (medium weight)
          - Keyword overlap with term-frequency weighting
        """
        ...
        query_bigrams = set(f"{query_words[i]}_{query_words[i+1]}"
                           for i in range(len(query_words)-1))
        # Also create merged form for OCR-split terms: "ph value" → also search "phvalue"
        query_merged = "".join(query_words)

        scored = []
        for bid, book in self.books.items():
            for entry in book["page_index"]:
                score = 0.0
                page_text = entry["text_lower"]

                # Signal 1: Exact phrase match (strongest — 10 points)
                if query_lower in page_text:
                    score += 10.0
                # Also check merged form in case PDF has no spaces
                elif len(query_merged) >= 3 and query_merged in page_text.replace(" ", ""):
                    score += 8.0

                # Signal 2: Bigram matches (5 points each)
                if query_bigrams:
                    bigram_hits = len(query_bigrams & entry["bigrams"])
                    score += bigram_hits * 5.0

                # Signal 3: Keyword overlap with frequency weighting
                for word in query_word_set:
                    if len(word) < 3:
                        # Short words: require exact word boundary match
                        # Count occurrences as whole word (not substring)
                        count = len(re.findall(r'\b' + re.escape(word) + r'\b', page_text))
                        if count > 0:
                            score += min(count, 5) * 1.5  # cap at 5 occurrences
                    else:
                        # Normal words: set membership + frequency bonus
                        if word in entry["keywords"]:
                            count = page_text.count(word)
                            score += 1.0 + min(count, 10) * 0.3
        ...
        results = sorted(scored, key=lambda x: -x["score"])[:top_k]
        # Debug: print scoring
        for r in results:
            print(f"  [RAG-PAGE] page {r['page']}: score={r['score']:.1f} preview={r['preview'][:60]}")
        return results

    def search(self, query, top_k=3):
        """Stage 2/3 retrieval: returns full content chunks for teaching.
        Same improved scoring as search_pages."""
        query_lower = query.lower().strip()
        query_words = re.findall(r"\w+", query_lower)
        ...
        for bid, book in self.books.items():
            for chunk in book["chunks"]:
                ...
                if query_lower in chunk_lower:
                    score += 10.0
                elif len(query_merged) >= 3 and query_merged in chunk_lower.replace(" ", ""):
                    score += 8.0

                # Keyword overlap with frequency
                for word in query_word_set:
                    if len(word) < 3:
                        count = len(re.findall(r'\b' + re.escape(word) + r'\b', chunk_lower))
                        if count > 0:
        ...
        results = sorted(scored, key=lambda x: -x["score"])[:top_k]
        for r in results:
            print(f"  [RAG-CHUNK] page {r['page']}: score={r['score']:.1f} text={r['text'][:60]}")
        return results
...
# ═══════════════════════════════════
# Deterministic Stage 1 Message Builder
# ═══════════════════════════════════

def build_stage1_message(topic: str, page_results: list, book_name: str) -> str:
    """Build a deterministic, fixed-text message for textbook Stage 1.
    No LLM involved — pure application logic."""

    if not page_results:
        return (
            f"Let's explore {topic} together! "
            f"I couldn't find the exact page in your textbook, "
            f"but try looking through the table of contents for a section about {topic}. "
            f"Read what you find and tell me what you learned!"
        )

    pages = sorted(set(r["page"] for r in page_results))

    if len(pages) == 1:
        page = pages[0]
        preview = page_results[0].get("preview", "")
        # Include a hint about what section to look at, if we have a preview
        if preview and len(preview) > 10:
            section_hint = f' Look for the section that starts with "{preview[:60]}."'
        else:
            section_hint = " Read through that section carefully."
        return (
            f"Great question! Let's find the answer together in your textbook. "
            f"Please open your book to page {page}.{section_hint} "
            f"Take a moment to read about {topic}, "
            f"and then tell me what you found!"
        )
    else:
        page_list = ", ".join(str(p) for p in pages[:-1]) + f" and {pages[-1]}"
        return (
            f"Great question! Let's find the answer in your textbook. "
            f"You can find information about {topic} on pages {page_list}. "
            f"Start with page {pages[0]} and read that section. "
            f"Then tell me what you learned!"
        )


# ═══════════════════════════════════
# System Instructions — Stage 2 & 3 only for textbook
# ═══════════════════════════════════
...
Give feedback, then provide a clear concise explanation. 2-3 sentences."""
    ...
    # ══════════════════════════════════════════════════════
    # TEXTBOOK MODE — Stage 2 and Stage 3 ONLY
    # Stage 1 is handled deterministically (no Gemini)
    # ══════════════════════════════════════════════════════
    if phase == PHASE_TEXTBOOK:
        ...
        # Stage 1 should NEVER reach here — it's handled deterministically
        # But just in case, return a safe fallback
        if stage == 1:
            print("[WARNING] instr_for_phase called for textbook stage 1 — this should not happen!")
            return f"{PERSONA}\nSay: 'Please open your textbook and tell me what you find about {topic}.'"
...
class LiveSessionBridge:
    def __init__(self, ws: WebSocket, device_id: str = "default"):
        ...
        self.last_rag = []
        self.last_page_results = []  # Stage 1 page locator results
    ...
    async def _open(self, instruction: str):
        ...
        for attempt in range(max_retries):
            try:
                ...
                self._activity_started = False
                print(f"[v5.0] Gemini session opened | phase={self.phase} stage={self.stage}")
                return
            except Exception as e:
                print(f"[v5.0] _open attempt {attempt+1}/{max_retries} failed: {e}")
                if attempt < max_retries - 1:
                    wait = 2 ** (attempt + 1)
                    print(f"[v5.0] retrying in {wait}s...")
                    await asyncio.sleep(wait)
                else:
                    print(f"[v5.0] _open failed after {max_retries} attempts")
        ...
        try:
            async for resp in self._live_session.receive():
                try:
                    ...
                    if sc.input_transcription:
                        ...
                        if t:
                            ...
                            if self.phase == PHASE_AWAITING_MODE and not self._yes_no_detected:
                                ...
                                if is_yes(full):
                                    print(f"[v5.1] transcript YES: {full!r}")
                                    await self._handle_yes_no("yes")
                                    return
                                elif is_no(full):
                                    print(f"[v5.1] transcript NO: {full!r}")
                                    await self._handle_yes_no("no")
                                    return

                            # ═══════════════════════════════════════════════
                            # TOPIC CAPTURE: Early interception.
                            # Fire ONLY after user stops speaking (ActivityEnd sent)
                            # and we have enough transcript to extract a topic.
                            # This races against generation_complete — whichever
                            # fires first wins and does deterministic Stage 1.
                            # ═══════════════════════════════════════════════
                            if (self.phase == PHASE_TOPIC_CAPTURE
                                    and not self._topic_intercepted
                                    and not self._activity_started  # user stopped speaking
                                    and len(clean_transcript(self._accumulated_user_text)) >= 4):
                                self._topic_intercepted = True
                                full = clean_transcript(self._accumulated_user_text)
                                ...
                                if self.topic:
                                    print(f"[v5.1] EARLY INTERCEPT topic_capture → topic={self.topic!r}")
                                    self.stage = 1
                    ...
                    # We only want input_transcription (ASR).
                    # Drop any audio/output the model tries to generate.
                    # ═══════════════════════════════════════════════
                    if self.phase == PHASE_TOPIC_CAPTURE:
                        # Suppress output transcription entirely
                        if sc.output_transcription:
                            suppressed = (sc.output_transcription.text or "").strip()
                            if suppressed:
                                print(f"[v5.0] SUPPRESSED topic_capture output: {suppressed[:80]!r}")
                        # Suppress model audio entirely (don't buffer, don't send)
                        if sc.model_turn:
                            print(f"[v5.0] SUPPRESSED topic_capture audio chunk")
                        # Skip to generation_complete handling (fall through)
                    else:
                        # Normal phases: forward output transcription and audio
                        if sc.output_transcription:
                    ...
                    if sc.generation_complete:
                        ...
                        if self.phase != PHASE_TOPIC_CAPTURE:
                            ...
                            # Discard any buffered audio from topic_capture
                            self._audio_buf = bytearray()
                        ...
                        if self.phase == PHASE_CURIOSITY and not self.topic and full:
                            ...
                            if self.topic:
                                await self._send({"type": "topic", "value": self.topic})
                                print(f"[v5.0] curiosity topic: {self.topic!r}")
                        ...
                        if self.phase == PHASE_TEXTBOOK_READY:
                            ...
                            print(f"[v5.0] textbook_ready done → topic_capture")

                        rag_pages = sorted(set(r["page"] for r in self.last_rag)) if self.last_rag else []
                        # Also include page results from Stage 1
                        if not rag_pages and self.last_page_results:
                            rag_pages = sorted(set(r["page"] for r in self.last_page_results))
                        await self._close()
                        print(f"[v5.0] Turn {self.turn_count} done{' (TOPIC DONE)' if was_final else ''} "
                              f"stage→{self.stage}")
                        ...
                        if was_final and self.topic:
                            ...
                            print(f"[v5.0] recorded topic '{self.topic}' to session store")
                    ...
                    print(f"[v5.0] recv error: {e}")
                    import traceback; traceback.print_exc()
                    await self._send({"type": "error", "message": str(e)})
                    return
        except asyncio.CancelledError: raise
        except Exception as e:
            print(f"[v5.0] recv outer error: {e}")
            import traceback; traceback.print_exc()
    ...
    def _infer_topic(text: str) -> str:
        ...
        known_terms = {
            "phvalue": "ph value", "phevalue": "ph value",
            "ph": "ph", "phscale": "ph scale",
            "phskill": "ph scale", "phscal": "ph scale",  # ASR misrecognitions
            "phskale": "ph scale", "pscale": "ph scale",
    ...
    async def begin_turn(self):
        ...
        # ══════════════════════════════════════════════════════
        # TEXTBOOK STAGE 1: Deterministic — skip Gemini entirely
        # ══════════════════════════════════════════════════════
        if self.phase == PHASE_TEXTBOOK and self.stage == 1 and self.topic:
            await self._handle_textbook_stage1()
            return  # Turn is complete, no Gemini session needed

        # ══════════════════════════════════════════════════════
        # TOPIC CAPTURE: Use Gemini ONLY for ASR transcription.
        # The _recv_loop will intercept the transcript and
        # immediately close Gemini + trigger deterministic Stage 1
        # BEFORE Gemini can generate any response audio.
        # ══════════════════════════════════════════════════════
        if self.phase == PHASE_TOPIC_CAPTURE:
            print(f"[v5.1] begin_turn #{self.turn_count} phase=topic_capture (ASR-only mode)")
            # Use a config that requests TEXT response (not audio) to reduce output
            instr = ("You are a silent transcription service. "
                     "DO NOT speak. DO NOT generate audio. "
                     "Only transcribe what the user says. Stay silent.")
            await self._open(instr)
            print(f"[v5.1] topic_capture: Gemini opened for ASR only, waiting for user audio")
            return  # Not speak-first, user will send audio

        # For Stage 2/3 textbook mode, do full content retrieval
        rag = []
        if self.phase == PHASE_TEXTBOOK and self.topic:
            rag = textbook_store.search(self.topic, top_k=3)
            self.last_rag = rag
            print(f"[v5.1] RAG search for '{self.topic}': {len(rag)} chunks found")
            for r in rag:
        ...
        print(f"[v5.1] begin_turn #{self.turn_count} phase={self.phase} stage={self.stage}")
        print(f"[v5.1] system instruction preview: {instr[:200]}...")
        await self._open(instr)

        # ── Speak-first logic (no longer includes textbook stage 1) ──
        TRIGGERS = {
        ...
        if speak_first:
            ...
            if self._live_session:
                try:
                    ...
                    print(f"[v5.1] sent trigger: {trigger[:50]}")
                except Exception as e:
                    print(f"[v5.1] trigger error: {e}")
        else:
            print(f"[v5.1] waiting for user audio")
    ...
    async def send_audio(self, pcm: bytes, mime: str = "audio/pcm;rate=16000"):
        ...
        if self._audio_msg_count <= 3 or self._audio_msg_count % 100 == 0:
            print(f"[v5.0] audio #{self._audio_msg_count} session={'OK' if self._live_session else 'NONE'}")
        ...
        if not self._activity_started:
            ...
            try:
                ...
                print(f"[v5.0] >>> ActivityStart")
            except Exception as e:
                print(f"[v5.0] ActivityStart err: {e}")
        try:
            await self._live_session.send_realtime_input(audio=types.Blob(data=pcm, mime_type=mime))
        except Exception as e:
            if self._audio_msg_count <= 5: print(f"[v5.0] audio send err: {e}")

    async def end_turn(self):
        print(f"[v5.0] end_turn() session={'OK' if self._live_session else 'NONE'} activity={self._activity_started}")
        if self._live_session and self._activity_started:
            try:
                ...
                print(f"[v5.0] >>> ActivityEnd")
            except Exception as e:
                print(f"[v5.0] ActivityEnd err: {e}")
            self._activity_started = False

    async def set_phase(self, phase: str):
        self.phase = phase
        await self._send({"type": "phase", "value": phase})
        print(f"[v5.0] set_phase → {phase}")
    ...
    async def notify_upload(self, book_id: str, book_name: str):
        ...
        await self._send({"type": "phase", "value": self.phase})
        print(f"[v5.0] textbook uploaded → {self.phase}")
...
{{
  ...
  "knowledge": [
    ...
    try:
        ...
        print(f"[v5.0] summary generation error: {e}")
        knowledge = []
        for t in topics:
            knowledge.append({
                "topic": t["topic"], "mode": t["mode"],
...
@app.get("/")
async def root():
    return JSONResponse({"status": "ok", "version": "v5.1", "textbooks": textbook_store.list_books()})
...
async def live_ws(websocket: WebSocket):
    ...
    print(f"[v5.0] Pi connected: {device_id}")
    try:
        ...
        while True:
            ...
            if t == "hello":
                print(f"[v5.0] hello from {msg.get('device','?')}")
                await bridge._send({"type": "state", "value": "ready"})
                await bridge._send({"type": "phase", "value": bridge.phase})

            elif t == "start_turn":
                print(f"[v5.0] start_turn phase={bridge.phase}")
                try:
                    await bridge.begin_turn()
                    # Only send listening if we actually opened a Gemini session
                    # (textbook stage 1 is deterministic and already completed)
                    if bridge._live_session is not None:
                        await bridge._send({"type": "state", "value": "listening"})
                    print(f"[v5.0] begin_turn OK")
                except Exception as e:
                    import traceback; print(f"[v5.0] begin_turn ERR: {e}"); traceback.print_exc()
                ...
                print(f"[v5.0] end_turn from Pi, phase={bridge.phase}")
                if bridge.phase not in (PHASE_GREETING, PHASE_AWAITING_UPLOAD):
                    await bridge.end_turn()
                else:
                    print(f"[v5.0] skipping end_turn (always speak-first)")
                ...
                print(f"[v5.0] vosk: {msg.get('answer')!r}")
    ...
    except WebSocketDisconnect: print(f"[v5.0] disconnected: {device_id}")
    except Exception as e:
        print(f"[v5.0] error: {e}"); import traceback; traceback.print_exc()
    finally:
